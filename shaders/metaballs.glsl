...
{
	float t=u_time*0.5*light_speed_modifier;
...
void main()
{
	...
	float t = u_time*.5*object_speed_modifier + 2.0;
	...
	p = rotate_x(p,u_mouse.y*9.0+offset1);
	p = rotate_y(p,u_mouse.x*9.0+offset2);
	vec3 d = vec3(uv,1.0);
	d.z -= length(d)*.5; //lens distort
	d = normalize(d);
	d = rotate_x(d,u_mouse.y*9.0+offset1);
	d = rotate_y(d,u_mouse.x*9.0+offset2);
